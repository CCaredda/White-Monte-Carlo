...
% Save constants
if ~ isfile('output/cst.mat')
    save('output/cst.mat','info_model');
end
...
% 6: Activated capillaries
cfg.prop=[0 0 1 1; ...
opt_prop.mua_GM opt_prop.mus_GM opt_prop.g_GM opt_prop.n_GM ; ...
opt_prop.mua_LBV opt_prop.mus_LBV opt_prop.g_LBV opt_prop.n_LBV ; ...
opt_prop.mua_Cap opt_prop.mus_Cap opt_prop.g_Cap opt_prop.n_Cap ; ...
opt_prop.mua_act_GM opt_prop.mus_act_GM opt_prop.g_act_GM opt_prop.n_act_GM ; ...
opt_prop.mua_act_LBV opt_prop.mus_act_LBV opt_prop.g_act_LBV opt_prop.n_act_LBV ; ...
opt_prop.mua_act_Cap opt_prop.mus_act_Cap opt_prop.g_act_Cap opt_prop.n_act_Cap];
...
% save output
writematrix(output_det.nscat,strcat('output/nscat_',num2str(Lambdas),'.txt'));
writematrix(output_det.ppath,strcat('output/ppath_',num2str(Lambdas),'.txt'));
writematrix(output_det.p,strcat('output/p_',num2str(Lambdas),'.txt'));
writematrix(output_det.v,strcat('output/v_',num2str(Lambdas),'.txt'));
writematrix(output_det.prop,strcat('output/prop_',num2str(Lambdas),'.txt'));
...
% Zip files
cd output/;
zip(strcat(num2str(Lambdas),'.zip'),{strcat('nscat_',num2str(Lambdas),'.txt'), ...
strcat('ppath_',num2str(Lambdas),'.txt'), ...
strcat('p_',num2str(Lambdas),'.txt'), ...
strcat('v_',num2str(Lambdas),'.txt'), ...
strcat('prop_',num2str(Lambdas),'.txt')});

% remove txt files
delete(strcat('nscat_',num2str(Lambdas),'.txt'));
delete(strcat('ppath_',num2str(Lambdas),'.txt'));
delete(strcat('p_',num2str(Lambdas),'.txt'));
delete(strcat('v_',num2str(Lambdas),'.txt'));
delete(strcat('prop_',num2str(Lambdas),'.txt'));

cd ..;

% if cfg.issaveref
%     dref = flux.dref(:,:,1);
%     save(strcat('output/out_',num2str(Lambdas),'nm.mat'),'output_det','dref', '-v7.3');
% else
%     save(strcat('output/out_',num2str(Lambdas),'nm.mat'),'output_det', '-v7.3');
% end
